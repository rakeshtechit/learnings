...
Ensure that the design is detailed specification is created. The specification should be broken done based on achitecture sub-components of modules, so that the development can be carried out in parallel. Includes technical details depecting the application flow inputs/ouputs and integration points to each sub-routine is captured.

Pass the design as a whole to the coder agent, so that it can understand and build and develop the functionalities based on the designer